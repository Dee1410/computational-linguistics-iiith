...
function englishs1() {
	...
	var x = Math.floor(Math.random()*e.length);
	var q =e[x];
	var r = s(q);
	button(r);
}
...
function s(n) {
  ...
  for(var i=0 ; i<=pq.length-1 ; ++i) {
    var j = Math.floor(Math.random()*pq.length);      
    var tep = pq[i];            
    pq[i] = pq[j];
    pq[j] = tep;
  }
...
function button(n) {
 ...
 $(".qs").click(function(){
			$(this).hide()
    var z = $(this).text();
			document.getElementById("fm1").innerHTML += z + " ";
			document.getElementById("cr").innerHTML = "<b>Formed Sentence</b>"+"<i>(after selection)</i>";
			$("#gf").show();
